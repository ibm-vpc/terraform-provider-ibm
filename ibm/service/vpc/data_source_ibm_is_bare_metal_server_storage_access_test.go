...
func TestAccIBMISBMSStorageAccessDataSource_basic(t *testing.T) {
	resName := "data.ibm_is_bare_metal_server_storage_access.test1"
	var server string
...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMISBMSStorageAccessDataSourceConfig(vpcname, subnetname, sshname, publicKey, name),
...
func testAccCheckIBMISBMSStorageAccessDataSourceConfig(vpcname, subnetname, sshname, publicKey, name string) string {
	return testAccCheckIBMISBareMetalServerConfig(vpcname, subnetname, sshname, publicKey, name) + `
	data "ibm_is_bare_metal_server_storage_access" "test1" {
