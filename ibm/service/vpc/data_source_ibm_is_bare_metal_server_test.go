...
func TestAccIBMISBMSDataSource_gpu(t *testing.T) {
	var server string
	resName := "data.ibm_is_bare_metal_server.test1"
	vpcname := fmt.Sprintf("tf-vpc-%d", acctest.RandIntRange(10, 100))
	name := fmt.Sprintf("tf-server-%d", acctest.RandIntRange(10, 100))
...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMISBMSDataSourceGPUConfig(vpcname, subnetname, sshname, publicKey, name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMISBareMetalServerExists("ibm_is_bare_metal_server.testacc_bms", server),
					resource.TestCheckResourceAttrSet(resName, "gpu.#"),
					resource.TestCheckResourceAttrSet(resName, "gpu.0.count"),
					resource.TestCheckResourceAttrSet(resName, "gpu.0.manufacturer"),
					resource.TestCheckResourceAttrSet(resName, "gpu.0.memory"),
					resource.TestCheckResourceAttrSet(resName, "gpu.0.model"),
				),
			},
		},
	})
...
func testAccCheckIBMISBMSDataSourceGPUConfig(vpcname, subnetname, sshname, publicKey, name string) string {
	return testAccCheckIBMISBareMetalServerGPUConfig(vpcname, subnetname, sshname, publicKey, name) + fmt.Sprintf(`
		data "ibm_is_bare_metal_server" "test1" {
			name = "%s"
		}`, name)
}
