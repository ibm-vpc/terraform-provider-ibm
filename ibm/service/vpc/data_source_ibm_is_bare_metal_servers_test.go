...
func TestAccIBMISBMSsDataSource_gpu(t *testing.T) {
	resName := "data.ibm_is_bare_metal_servers.test1"
	var server string
	vpcname := fmt.Sprintf("tf-vpc-%d", acctest.RandIntRange(10, 100))
	name := fmt.Sprintf("tf-server-%d", acctest.RandIntRange(10, 100))
...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMISBMSsDataSourceGPUConfig(vpcname, subnetname, sshname, publicKey, name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMISBareMetalServerExists("ibm_is_bare_metal_server.testacc_bms", server),
					resource.TestCheckResourceAttrSet(resName, "servers.0.gpu.#"),
				),
			},
		},
	})
...
func testAccCheckIBMISBMSsDataSourceGPUConfig(vpcname, subnetname, sshname, publicKey, name string) string {
