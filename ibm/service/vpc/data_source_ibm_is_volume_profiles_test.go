...
func TestAccIBMISVolumeProfilesDataSource_supportedAttachmentModes(t *testing.T) {
	resName := "data.ibm_is_volume_profiles.test1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet(resName, "profiles.0.name"),
					resource.TestCheckResourceAttrSet(resName, "profiles.0.family"),
					resource.TestCheckResourceAttrSet(resName, "profiles.0.supported_attachment_modes.#"),
					resource.TestCheckResourceAttrSet(resName, "profiles.0.supported_attachment_protocols.#"),
				),
			},
		},
	})
