...
func TestResourceIBMISInstanceRescueMapToInstanceRescuePrototypeInstanceRescueByImage(t *testing.T) {
	checkResult := func(result *vpcv1.InstanceRescuePrototypeInstanceRescueByImage) {
		keyIdentityModel := new(vpcv1.KeyIdentityByID)
		keyIdentityModel.ID = core.StringPtr("r006-82679077-ac3b-4c10-be16-63e9c21f0f45")
		...
		model := new(vpcv1.InstanceRescuePrototypeInstanceRescueByImage)
		model.Keys = []vpcv1.KeyIdentityIntf{keyIdentityModel}
		model.UserData = core.StringPtr("[...]")
		model.Image = imageIdentityModel
		model.RescueVolumeAttachment = instanceRescueVolumeAttachmentPrototypeModel
	...
	result, err := vpc.ResourceIBMISInstanceRescueMapToInstanceRescuePrototypeInstanceRescueByImage(model)
	assert.Nil(t, err)
	checkResult(result)
}
