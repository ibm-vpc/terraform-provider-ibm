...
description: |-
  Get information about an IBM bare metal server volume attachment.
---

# ibm\_is_bare_metal_server_volume_attachment

Import the details of an existing IBM Cloud Bare Metal Server volume attachment as a read-only data source. For more information, about managing VPC Bare Metal Server, see [About Bare Metal Servers for VPC](https://cloud.ibm.com/docs/vpc?topic=vpc-about-bare-metal-servers).
...
```terraform
data "ibm_is_bare_metal_server_volume_attachment" "example" {
...
- `name` - (Required, String) The name for this volume attachment. The name is unique across all volume attachments on the bare metal server.
