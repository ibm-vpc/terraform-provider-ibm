...
* `cluster_network_attachments` - (List) Collection of instance cluster network attachments. A cluster network attachment represents a device to which a cluster network interface is attached.The collection is sorted based on the `before` property.
