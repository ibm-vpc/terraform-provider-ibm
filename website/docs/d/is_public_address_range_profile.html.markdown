---
layout: "ibm"
page_title: "IBM : ibm_is_public_address_range_profile"
description: |-
  Get information about PublicAddressRangeProfile
subcategory: "Virtual Private Cloud API"
---

# ibm_is_public_address_range_profile

Provides a read-only data source to retrieve information about a PublicAddressRangeProfile. You can then reference the fields of the data source in other resources within the same configuration by using interpolation syntax.

## Example Usage

```hcl
data "ibm_is_public_address_range_profile" "is_public_address_range_profile" {
	name = "user-ipv4"
}
```
...
* `name` - (Required, Forces new resource, String) The public address range profile name.
...
* `id` - The unique identifier of the PublicAddressRangeProfile.
...
Nested schema for **targetable_resource_types**:
  ...
  * `values` - (List) The resource types that public address ranges with this profile can target. If empty, the public address range cannot target a resource.
    * Constraints: Allowable values are: `load_balancer`,`virtual_network_interface`, `vpc`.
