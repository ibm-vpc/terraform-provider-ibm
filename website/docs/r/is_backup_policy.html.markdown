...
- `included_content` - (Optional, List) The included content for backups created using this policy. Allowed values depend on `match_resource_type`:
  - When `match_resource_type` is `instance` (default): `boot_volume` (include the instance's boot volume), `data_volumes` (include the instance's data volumes).
  - When `match_resource_type` is `bare_metal_server`: only `data_volumes` is supported (include the bare metal server's data volumes).
...
- `match_resource_type` - (Optional, String) The resource type this backup policy will apply to. Resources that have both a matching type and a matching user tag will be subject to the backup policy. The default value is `["volume"]`. Allowed values are `volume`,`instance`,`share`,`bare_metal_server`.
