...
- `name` - (Optional, String) The name for this network attachment. Names must be unique within the bare metal server the network attachment resides in. If unspecified, the name will be a hyphenated list of randomly-selected words.
- `virtual_network_interface` - (Required, List) The virtual network interface for this network attachment.
...
- `bare_metal_server_network_attachment_id` - (String) The unique identifier for this network attachment.
  - Constraints: The maximum length is `64` characters. The minimum length is `1` character. The value must match regular expression `/^[-0-9a-z_]+$/`.
- `created_at` - (String) The date and time that the network attachment was created.
...
- `id`: A string. The network attachment identifier.
